...
def myfft():
    ...
    csv = readCSV.readCSV()
    rawdata = csv.csv_read('DATA_raw.csv')
    notchdata = csv.csv_read('DATA_notch.csv')
    gain100 = csv.csv_read('DATA_openloop_gain100.csv')
    gain1000 = csv.csv_read('DATA_openloop_gain1000.csv')
    gain1000notch = csv.csv_read('DATA_openloop_gain1000_notch.csv')
    lpf500ok = csv.csv_read('DATA_lpf500_ok.csv')
    lpf1000ok = csv.csv_read('DATA_lpf1000_ok.csv')
    lpf1000ol = csv.csv_read('DATA_lpf100_notch1_ol.csv')
    yaxis300 = csv.csv_read('DATA_Yaxis_300.csv')
    ...
    for i in range(0, len(data)):
        ...
        # 高速フーリエ変換
        F1 = fft(data[i][3].values)

        # 振幅スペクトルを計算
        Amp1 = np.abs(F1)

        plt.plot(freq, Amp1, label=label[i])
    # plt.legend()
