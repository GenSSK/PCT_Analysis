...
class Iden:
    def npz_load(self):
        ...
        self.dir = "/Volumes/GoogleDrive/My Drive/program/ARCS-PCT/data/PCT_test/"  # mac
        # self.data = np.load(self.dir + '2021-10-27_BilateralTest.npz')
        self.data = np.load(self.dir + '2021-12-06_4daitest.npz')
    ...
    def graph_sub(self):
        ...
        fig, (thr, text) = plt.subplots(2, 1, figsize=(10, 10), sharex=True)

        # plt.xlim([20, 20.01])  # x軸の範囲
        plt.xlim([0.0, 45.0])  # x軸の範囲
        plt.xlabel("Time[sec]")

        thr.plot(self.data['time'], self.data['i1_r_thm'], label='Interface1')
        thr.plot(self.data['time'], self.data['i2_r_thm'], label='Interface2')
        thr.plot(self.data['time'], self.data['i3_r_thm'], label='Interface3')
        thr.plot(self.data['time'], self.data['i4_r_thm'], label='Interface4')
        thr.set_ylabel('Position [rad]')
        thr.legend()
        thr.set_yticks(np.arange(-10, 10, 0.5))
        thr.set_ylim([-0.5, 0.5])  # y軸の範囲
        #
        ...
        text.plot(self.data['time'], self.data['i1_r_tdish'], label='Interface1')
        text.plot(self.data['time'], self.data['i2_r_tdish'], label='Interface2')
        text.plot(self.data['time'], self.data['i3_r_tdish'], label='Interface3')
        text.plot(self.data['time'], self.data['i4_r_tdish'], label='Interface4')
