...
class each:
    ...
    def period_performance(self, graph=1):
        ...
        error_period_array = np.vstack(([_ for _ in error_period]))
        spent_period_array = np.vstack(([_ for _ in spent_period]))
    ...
    def get_force(self):
        force = np.zeros((len(self.data), 2, self.join, self.end_num - self.start_num))
        for i in range(len(self.data)):
            data = self.data[i]
            for j in range(self.join):
                interfacenum = 'i' + str(j + 1)
                force[i][0][j] = data[interfacenum + '_p_text'][self.start_num:self.end_num]
                force[i][1][j] = data[interfacenum + '_r_text'][self.start_num:self.end_num]
    ...
    def get_force_diff_dyad_only(self):
        force = self.get_force()
    ...
    def get_force_effort(self):
        force_all = self.get_force_all()
        force_plate = self.get_plate_force()
        force_effort = force_all - force_plate
        return force_effort

    def get_performance(self):
        performance = np.zeros((len(self.data), self.end_num - self.start_num))
        for i in range(len(self.data)):
            data = self.data[i]
    ...
    def get_time_to_target(self):
        time_to_target = np.zeros((len(self.data), self.period))
        reaching = np.zeros((len(self.data), self.end_num - self.start_num))
        r = 0.03
        for i in range(len(self.data)):
            data = self.data[i]
            error = np.sqrt(
            ...
            reaching[i] = np.where(error < r, 0, 1)
            reaching_ = reaching[i].reshape([self.period, self.num])

            for j in range(self.period):
                time_to_target[i][j] = np.count_nonzero(reaching_[j]) * self.smp

        # plt.plot(np.arange(self.start_num, self.end_num, 1) ,reaching[0])
        # plt.ylim([-1, 2])
        # plt.plot(np.arange(self.period) + 1, time_to_target[0])
        # plt.show()
        return time_to_target

    def get_pq_diff(self):
        csv = pd.read_csv('csv/PQ_calc.csv')
        subject_list = []
        for i in range(len(self.data)):
            subject_list.append([])
            data = self.data[i]
            subject_list[i].append(''.join(chr(char) for char in data['j1_subject']))
            subject_list[i].append(''.join(chr(char) for char in data['j2_subject']))
        group_type = self.group_type
        if group_type == 'AdPD':
            group_type = 'AD'
        if group_type == 'Bi':
            group_type = '4C'
        csv = csv[csv['type'] == group_type]
        # print(csv)
        invc_diff = np.zeros(len(self.data))
        natrl_diff = np.zeros(len(self.data))
        ifqual_diff = np.zeros(len(self.data))

        for i in range(len(self.data)):
            data = self.data[i]
            # print(subject_list[i][0])
            csv_name = csv[csv['name'] == subject_list[i][0]]
            # print(csv_name)
            # print(subject_list[i][1])
            csv1 = csv_name[csv_name['partner'] == subject_list[i][1]]
            csv_name = csv[csv['name'] == subject_list[i][1]]
            # print(csv_name)
            csv2 = csv_name[csv_name['partner'] == subject_list[i][0]]

            invc_diff[i] = np.abs(csv1['INV/C'].values - csv2['INV/C'].values)
            natrl_diff[i] = np.abs(csv1['NATRL'].values - csv2['NATRL'].values)
            ifqual_diff[i] = np.abs(csv1['IFQUAL'].values - csv2['IFQUAL'].values)
        return np.array((invc_diff, natrl_diff, ifqual_diff))

    def get_cooperation_diff(self):
        csv = pd.read_csv('csv/Cooperation_calc.csv')
        subject_list = []
        for i in range(len(self.data)):
            subject_list.append([])
            data = self.data[i]
        ...
        csv = csv[csv['type'] == group_type]
        # print(csv)
        per_diff = np.zeros(len(self.data))
        cop_diff = np.zeros(len(self.data))
        int_diff = np.zeros(len(self.data))
        ...
        for i in range(len(self.data)):
            ...
            per_diff[i] = np.abs(csv1['PER'].values - csv2['PER'].values)
            cop_diff[i] = np.abs(csv1['COP'].values - csv2['COP'].values)
            int_diff[i] = np.abs(csv1['INT'].values - csv2['INT'].values)
        return np.array((per_diff, cop_diff, int_diff))